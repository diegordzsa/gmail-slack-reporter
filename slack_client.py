...
import config
from gmail_client import ReportStats


def build_blocks(stats: ReportStats, report_date: date) -> list[dict]:
...
def send_report(stats: ReportStats, report_date: date) -> None:
    payload = {
        "text": (
            f"Daily Email Report — {stats.emails_received} received, "
            f"{stats.total_replies} replies sent, {stats.still_waiting} still waiting"
        ),
